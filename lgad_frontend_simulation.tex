...
it mostly relies numerical methods. We implement most calculations in the time domain, while the frequency domain is mostly used
to cross-check noise and the FEE expected response.

\subsubsection{front-end implementation}
We use a second order low-pass filter which transfer function
 ...
 The output pulse of the FEE is the convolution (in time domain) of the pulse coming from the LGAD library and the FEE impulse response.
 The time base for the pulses and the convolution is 1~\si{ps} -- this is the sampling time that we use for the whole
 signal simulation, noise is treated differently and cover in~\ref{sec:noise_simulation}.
