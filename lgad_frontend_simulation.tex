...
are: $1.5<\abs{\eta}<X.X$, and $2.4<\abs{\eta}<4.2$ for CMS and ATLAS, respectively. In
...
Our results indicate that for FEE analog bandwidths (BWs) larger than 350~\si{MHz},
corresponding to shaping times less than 1~\si{ns}, and signal-to-noise ratios
(SNR) larger than 30, time resolutions of 30--37~\si{ps} and 34--47~\si{ps}
...
For every point in this scan we evaluate the time resolution for LE and CFD.
...
The front-end simulation is implemented in c++ programing language. It combines analytical calculations when possible and 
numerical methods. We implement most of the calculations in the time domain, while the frequency domain is mostly used
...
thereof. The FEE is a second order low-pass filter with transfer function
and impulse response given by equations~\ref{eq:filter_tf} and~\ref{eq:filter_ir}, respectively.
...
a given voltage threshold has been crossed. The threshold can be a constant value (LE) or a constant fraction of the pulse height (CF).
...
20~\si{ps} binning while the time-over-thershold is measured with a 100~\si{ps} binning in order to simulate the effect of quantization. 
...
and linear coefficients of the 2th-oder polynomial fit. Different corrections are derived for each simulation scenario
...
$1\times 10^{15}$~n/cm$^2$. Overall our simulation results indicate that time resolutions better than 
